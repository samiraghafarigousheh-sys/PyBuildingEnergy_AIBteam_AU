...
def calc_humidity_ratio(T_db, RH, Patm=101325):
    """
    Converts Temp (C) and RH (%) to Humidity Ratio (kg/kg).
    """
    T_k = float(T_db) + 273.15
    # Saturation vapor pressure
    P_ws = np.exp(77.345 + 0.0057 * T_k - 7235 / T_k) / (T_k ** 8.2)
    # Actual vapor pressure
    P_w = (float(RH) / 100.0) * P_ws
    # Humidity ratio
    x = 0.62198 * P_w / (Patm - P_w)
    return x



def calc_basement_wall_u_value(U_wall_raw, lambda_gr, z):
    """
    ISO 13370 Basement Wall U-Value Calculation.
    Converts a standard 1D wall U-value into an effective soil-coupled U-value.
    
    :param U_wall_raw: The raw U-value of the concrete/brick wall alone [W/m2K]
    :param lambda_gr: Thermal conductivity of the soil [W/mK]
    :param z: Depth of the basement wall below ground level [m]
    """
    # 1. Calculate the raw thermal resistance of the wall (removing standard air films)
    R_si = 0.13  # Internal surface resistance
    R_se = 0.04  # External surface resistance (standard)
    R_w = (1.0 / float(U_wall_raw)) - R_si - R_se
    
    # Ensure resistance doesn't mathematically break if U_wall is unusually high
    if R_w < 0.001: R_w = 0.001 
    
    # 2. Calculate equivalent thickness (dt) of the wall in terms of soil
    d_t = lambda_gr * (R_si + R_w + R_se)
    
    # 3. ISO 13370 Equation for U_bw (Basement Wall)
    term1 = (2.0 * lambda_gr) / (np.pi * float(z))
    term2 = 1.0 + (0.5 * d_t) / (d_t + float(z))
    term3 = np.log((float(z) / d_t) + 1.0)
    
    U_bw = term1 * term2 * term3
    return U_bw
...
class ISO52016:
    ...
    def _Temperature_and_Energy_needs_calculation_core(
        ...
        **kwargs,
        ...
        # --- LATENT HEAT INITIALIZATION ---
        rho_air = 1.204  # kg/m3
        try:
            h_bui = float(building_object["building"]["height"])
            a_use = float(building_object["building"]["net_floor_area"])
        except KeyError:
            h_bui, a_use = 3.0, 100.0 # safe fallback
            
        vol_air = a_use * h_bui
        M_air = vol_air * rho_air # Total mass of air in the zone
        dt = 3600.0 # Time step in seconds
        
        x_air_old = 0.008 # Start simulation at standard ~50% RH
        
        X_air_indoor_act = np.zeros(Tstepn)
        Phi_Latent_nd_act = np.zeros(Tstepn)
        # ----------------------------------
        ...
        with tqdm(total=Tstepn) as pbar:
            ...
            for Tstepi in range(start_idx, Tstepn):
                ...
                while iterate:
                    ...
                    try:
                        occ_data = next((item for item in building_object["building_parameters"]["internal_gains"] if item["name"] == "occupants"), None)
                        num_people = float(occ_data["full_load"]) if occ_data else 0.0
                    except KeyError:
                        num_people = 0.0
                    
                    occupancy_fraction = float(profile_df['occupancy_profile'].iloc[Tstepi])
                    G_int_t = num_people * occupancy_fraction * 0.0000167
                    
                    # 2. Ventilation Moisture Transport
                    c_air = 1006.0
                    m_dot_vent = H_ve_nat / c_air # Convert W/K to mass flow kg/s
                    
                    T_ext = sim_df.iloc[Tstepi]["T2m"]
                    RH_ext = sim_df.iloc[Tstepi].get("RH", 50.0) # Default to 50% if EPW is missing RH
                    x_ext_t = calc_humidity_ratio(T_ext, RH_ext)
                    
                    # 3. Solve Implicit Mass Balance for Indoor Humidity
                    ...
                    x_air_old = x_air_t
                    X_air_indoor_act[Tstepi] = x_air_t
                    Phi_Latent_nd_act[Tstepi] = Latent_Load_W
                    # --------------------------------------------
        ...
        hourly_results = pd.DataFrame(
            data=np.vstack(
                (
                    ...
                    X_air_indoor_act[Tstep_first_act:],      # <-- NEW
                    Phi_Latent_nd_act[Tstep_first_act:],     # <-- NEW
                )
            ).T,
            index=sim_df[Tstep_first_act:].index,
            columns=["Q_HC", "T_op", "T_ext", "x_air_in", "Q_Latent"], # <-- NEW
        )
